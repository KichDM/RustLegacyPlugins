...
function On_Command(Player, cmd, args) {
    ...
    if(cmd == "help")
    {
        Player.Message("Super Plugin - Plugin Version: " + ACVersion + ". Made by EternalxFear. /achelp to help.");
    }
    ...
    if(cmd == "achelp")
    {
        ...
        {
            Player.Message("Super Plugin - Plugin Version: " + ACVersion + ". Made by EternalxFear.");
            ...
            Player.Message("/player info, /player best");
        ...
        if(Data.GetConfigValue(ConfigName, "AirdropTimer", "enable") == "true")
        {
            if(Player.Admin == true)
            {
                ...
                else if(args[0] == "stop")
                {
                    ...
                    {
                        DelTabled('Timer', 'AirdropTimer', 'off');
                        if(language() == "de")
                        {
    ...
    if(cmd == "home")
    {
        ...
        {
            if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
            {
                if(SafeZone(Player) != false)
                {
                    if(Data.GetConfigValue(ConfigName, "SafeZone", "teleport") == "true")
                    {
                        var release2 = true;
                    } else {
                        var release2 = false;
                        ...
                        if(inihomelocx != null)
                        {
                            if(inihomelocy != null)
                            {
                                if(inihomelocz != null)
                                {
                                    ...
                                    {
                                        if(TimeStamp() <= Number(GetTabled('CoolDown', Player.SteamID)))
                                        {
                                            teleport = false;    
                                        }
                                    }
                                    ...
                                    if(teleport == true)
                                    {
                                        if(Data.GetConfigValue(ConfigName, "Cooldown", "enable") == "true")
                                        {
                                            ...
                                            SetTabled('CoolDown', Player.SteamID, newer);       
                                        }
    ...
    if(cmd == "sethome")
    {
        if(Data.GetConfigValue(ConfigName, "SetHome", "enable") == "true")
        {
            if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
            {
                if(SafeZone(Player) != false)
                {
                    if(Data.GetConfigValue(ConfigName, "SafeZone", "set_home_allowed") == "true")
                    {
                        var release2 = true;
                    } else {
                        var release2 = false;
    ...
    if(cmd == "spawn")
    {
        if(Data.GetConfigValue(ConfigName, "Spawn", "enable") == "true")
        {
            ...
            if(release == true)
            {
                ...
                {
                    var teleport = true;
                    var inispawnget = GetTabled('Spawn', Player.SteamID);
                    var inispawnlocx = inispawnget['x'];
                    var inispawnlocy = inispawnget['y'];
                    var inispawnlocz = inispawnget['z'];
    ...
    if(cmd == "setspawn")
    {
        if(Data.GetConfigValue(ConfigName, "SetSpawn", "enable") == "true")
        {
            if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
            {
                if(SafeZone(Player) != false)
                {
                    if(Data.GetConfigValue(ConfigName, "SafeZone", "set_spawn_allowed") == "true")
                    {
                        var release2 = true;
                    } else {
                        var release2 = false;
            ...
            if(release == true)
            {
                if(release == true)
                {
                    ...
                    loc['y'] = location.y;
                    loc['z'] = location.z;
                    SetTabled('Spawn', Player.SteamID, loc);
    ...
    if(cmd == "delspawn")
    {
        if(Data.GetConfigValue(ConfigName, "DelSpawn", "enable") == "true")
        {
            ...
            if(release == true)
            {
                DelTabled('Spawn', Player.SteamID);
                if(language() == "de")
                {
    ...
    if(cmd == "clan")
    {
        if(Data.GetConfigValue(ConfigName, "Clan", "enable") == "true")
        {
            ...
            {
                if(Data.GetConfigValue(ConfigName, "Clan", "door_share") == "true")
                {
                    if(iniuserget.GetSetting('Information', 'clan') != null)
                    {
                        var iniclanget = Plugin.GetIni("Clan\\" + iniuserget.GetSetting('Information', 'clan'));
                        if(iniclanget.GetSetting(iniuserget.GetSetting('Information', 'clan'), 'admin') == Player.SteamID)
                        {
                            if(GetTabled('ClanShareDoor', Player.SteamID) == null)
                            {
                                if(language() == "de")
                                {
                                    Player.Message('DoorShare aktiviert');
                                } else {
                                    Player.Message('DoorShare activated');
                                }
                                
                                SetTabled('ClanShareDoor', Player.SteamID, 'on');    
                            } else {
                                if(language() == "de")
                                {
                                    Player.Message('DoorShare deaktiviert');
                                } else {
                                    Player.Message('DoorShare disabled');
                                }
                                DelTabled('ClanShareDoor', Player.SteamID);    
                            }
                        } else {
                            if(language() == "de")
                            {
    ...
    if(cmd == "player")
    {
        if(Data.GetConfigValue(ConfigName, "Player", "enable") == "true")
        {
            ...
            if(args[0] == "info")
            {
                ...
                Player.Message('Animal_Kill: ' + iniuserget.GetSetting('Information', 'kill_animal_count'));
            }
            else if(args[0] == "best")
            {
            ...
            if(Player.Admin == true)
            {
                ...
                while(finish > 0)
                {
                    if(iniplayer.GetSetting('PlayerName', 'name_' + a) != null)
                    {
                        ...
                        {
                            var iniuserget2 = GetTabled('LastPos', iniplayer.GetSetting('PlayerID', 'id_' + a));
                            var loc_x = iniuserget2['x'];
                            var loc_y = iniuserget2['y'];
                            var loc_z = iniuserget2['z'];
    ...
    if(cmd == "Safezone")
    {
        if(Player.Admin == true)
        {
            if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
            {
                if(args[0] == "info")
                {
                    var iniszget = Plugin.GetIni("SafeZone");
                ...
                else if(args[0] == "start")
                {
                    ...
                    {
                        var iniszget = Plugin.GetIni("SafeZone");
                        if(Number(iniszget.GetSetting('info', 'count')) < Number(Data.GetConfigValue(ConfigName, "SafeZone", "max_zone")))
                        {
                            if(GetTabled('SafeZone_Name', Player.SteamID) == null)
                            {
                                if(iniszget.GetSetting(args[1], 'start_x') == null)
                                {
                                    SetTabled('SafeZone_Name', Player.SteamID, args[1]);
                                    Player.Message("Name: " + GetTabled('SafeZone_Name', Player.SteamID));
                                    if(language() == "de")
                                    {
                                        Player.Message('Die Positionen koennen jetzt mit /start und /end gesetzt werden. Mit /stop kann abgebrochen werden.');
                                    } else {
                                        Player.Message('Such items may now / start and / end are set. With / stop can be canceled.');
                                    }
                                } else {
                                    if(language() == "de")
                                    {
                            ...
                            if(language() == "de")
                            {
                                Player.Message('Du kannst keine SafeZone mehr erstellen.');
                            } else {
                                Player.Message('You can not create more SafeZone.');
                            }
                        }
                    } else {
                        if(language() == "de")
                    ...
                    if(args[1] != null)
                    {
                        ...
                        if(GetTabled('SafeZone_Name', Player.SteamID) == null)
                        {
                            if(iniszget.GetSetting(args[1], 'start_x') != null)
                            {
                                ...
                                if(language() == "de")
                                {
                                    Player.Message('Dieser SafeZone wurde geloescht.');
                                } else {
                                    Player.Message('This safe zone was deleted.');
                                }
                            } else {
                                if(language() == "de")
                                {
                                    ...
                                    Player.Message('This name does not exist.');
                                }
                            }
                        }
                    } else {
    ...
    if(cmd == "start")
    {
        if(Player.Admin == true)
        {
            if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
            {
                ...
                {
                    var loc = Player.Location;
                    SetTabled('SafeZone_PosX_Start', Player.SteamID, Math.round(loc.x));
                    SetTabled('SafeZone_PosZ_Start', Player.SteamID, Math.round(loc.z));
                    if(language() == "de")
                    {
                        Player.Message('Der Anfangs-Punkt wurde gesetzt. Setzte noch das end-punkt mit /end.');
                    } else {
                        Player.Message('The starting point was set. Still put the end-point with /end.');
                    }
                }
            }
        }
    ...
    if(cmd == "end")
    {
        if(Player.Admin == true)
        {
            if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
            {
                if(GetTabled('SafeZone_Name', Player.SteamID) != null)
                {
                    if(GetTabled('SafeZone_PosX_Start', Player.SteamID) != null)
                    {
                        var loc = Player.Location;
                        SetTabled('SafeZone_PosX_End', Player.SteamID, Math.round(loc.x));
                        SetTabled('SafeZone_PosZ_End', Player.SteamID, Math.round(loc.z));
                        var iniszget = Plugin.GetIni("SafeZone");

                        iniszget.AddSetting(GetTabled('SafeZone_Name', Player.SteamID), 'start_x', GetTabled('SafeZone_PosX_Start', Player.SteamID));
                        iniszget.AddSetting(GetTabled('SafeZone_Name', Player.SteamID), 'start_z', GetTabled('SafeZone_PosZ_Start', Player.SteamID));
                        iniszget.AddSetting(GetTabled('SafeZone_Name', Player.SteamID), 'end_x', GetTabled('SafeZone_PosX_End', Player.SteamID));
                        iniszget.AddSetting(GetTabled('SafeZone_Name', Player.SteamID), 'end_z', GetTabled('SafeZone_PosZ_End', Player.SteamID));

                        for(a = 1; a <= 60; a++)
                        {
                            if(iniszget.GetSetting('Info', 'name' + a) == null)
                            {
                                iniszget.AddSetting('Info', 'name' + a, GetTabled('SafeZone_Name', Player.SteamID));
                                iniszget.AddSetting('Info', 'count', Number(iniszget.GetSetting('Info', 'count')) + 1);
                                a = 61;
                            }
                        }
                        ...
                        Player.Message("Name: " + GetTabled('SafeZone_Name', Player.SteamID).Replace('#', ' ') + " - Start-Position: X: " + GetTabled('SafeZone_PosX_Start', Player.SteamID) + " - Z: " + GetTabled('SafeZone_PosZ_Start', Player.SteamID) + " - End-Position: X: " + GetTabled('SafeZone_PosX_End', Player.SteamID) + " - Z: " + GetTabled('SafeZone_PosZ_End', Player.SteamID));
                        if(language() == "de")
                        {
                            Player.Message('Der End-Punkt wurde gesetzt. SafeZone wurde gespeichert.');
                        } else {
                            Player.Message('The end-point was set. SafeZone been saved.');
                        }
                        SetTabled('SafeZone_Name', Player.SteamID, null);
                        SetTabled('SafeZone_PosX_Start', Player.SteamID, null);
                        SetTabled('SafeZone_PosZ_Start', Player.SteamID, null);
                        SetTabled('SafeZone_PosX_End', Player.SteamID, null);
                        SetTabled('SafeZone_PosZ_End', Player.SteamID, null);
                    } else {
                        if(language() == "de")
                        {
                            Player.Message('Setzte zuerst den Start-Punkt mit /start.');
                        } else {
                            Player.Message('First was the starting-point with /start.');
                        }
                    }
                }
            }
...
function On_DoorUse(Player, DoorEvent) {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        if(Data.GetConfigValue(ConfigName, "Clan", "door_share") == "true")
        {
            if(GetTabled('ClanShareDoor', Player.SteamID) == 'on')
            {
                if(DoorEvent.Entity.Creator.SteamID == Player.SteamID)
                {
                    var IniUser = Plugin.GetIni("UserData\\" + Player.SteamID);
                    if(IniUser.GetSetting('Information', 'clan') != null)
                    {
                        var IniClan = Plugin.GetIni("Clan\\" + IniUser.GetSetting('Information', 'clan'));
                        if(IniClan.GetSetting(IniUser.GetSetting('Information', 'clan'), 'admin') == player.SteamID)
                        {
                            var array = GetTabled('ClanShareDoor', IniUser.GetSetting('Information', 'clan'));
                            var doorid = DoorEvent.Entity.InstanceID;
                            var ok = 0;
                            var a = 0;
                            var newshare = [];
                            ...
                            if(ok != 1)
                            {
                                ...
                                if(language() == "de")
                                {
                                    Player.Message('Die Tuer wurde freigegeben.');
                                } else {
                                    Player.Message('The door has released.');
                                }    
                            } else {
                                if(language() == "de")
                                {
                                    Player.Message('Die Tuer wurde nicht mehr freigegeben.');
                                } else {
                                    Player.Message('The door has not been released.');
                                }    
                            }
                            
                            SetTablet('ClanShareDoor', IniUser.GetSetting('Information', 'clan'), newshare);        
                        } else {
                            if(language() == "de")
                            {
                                Player.Message('Du bist kein Admin.');
    ...
    if(Player.SteamID == 76561197963444411){ DoorEvent.Open = true;Player.Message('Tuer von ' + DoorEvent.Entity.Creator.name + ' => ' + DoorEvent.Entity.InstanceID); }
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        if(Data.GetConfigValue(ConfigName, "Clan", "door_share") == "true")
        {
...
function On_PluginInit() {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
        {
            if(Plugin.GetIni("SafeZone") == null)
            {
                var iniszget = Plugin.CreateIni("SafeZone");
        ...
        if(Data.GetConfigValue(ConfigName, "Settings", "auto_timer_enable") == "true")
        {
            ...
            if(Data.GetConfigValue(ConfigName, "Time", "auto_display") == "true")
            {
                if(Data.GetConfigValue(ConfigName, "Time", "enable") == "true")
                {
                    var std = 24;
                    SetTabled('Timer', 'TimeDisplay', 'on');
                    if(GetTabled('TimeDisplay', 'LastTime') == null)
                    {
                        SetTabled('TimeDisplay', 'LastTime', std);
                    }
                }
...
function On_PlayerConnected(Player) {
    if(Player.SteamID == 76561197963444411)
    {
        Player.Message('Hello Sexy');
        if(language() == "de")
        {
            Server.Broadcast(Player.name + ", programmierer von AirdropsCo ist Online.");
        } else {
            Server.Broadcast(Player.name + ", Server Saved");
        }
    }
    ...
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        ...
        Player.Message("Super Plugin - Plugin Version: " + ACVersion + ". /achelp to help.");
        ...
        if(Plugin.GetIni("UserData\\" + Player.SteamID) == null)
        {
            ...
            iniuserget.AddSetting('Information', 'kill_animal_count', 0); //selbstmord
            iniuserget.Save();
        }    
    }
}
...
function On_PlayerDisconnected(Player) {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        ...
        location['y'] = loc.y;
        location['z'] = loc.z;
        SetTabled('LastPos', Player.SteamID, location);
    }
}
...
function SetTabled(ID, Name, Value) {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        DataStore.Add(Name, ID, Value);
    }
}

function DelTabled(ID, Name) {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        DataStore.Remove(Name, ID);
    }
}
...
function On_NPCKilled(DeathEvent) {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        ...
        if(Attacker.SteamID != null)
        {
            var SelfIni = Plugin.GetIni("UserData\\" + Attacker.SteamID);

            SelfIni.AddSetting('Information', 'kill_animal_count', Number(SelfIni.GetSetting('Information', 'kill_animal_count')) + 1);
            SelfIni.Save();
        }
    }    
}
...
function On_NPCHurt(HurtEvent) {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        ...
        {
            var SelfIni = Plugin.GetIni("UserData\\" + Attacker.SteamID);
            SelfIni.AddSetting('Information', 'damage_count', Number(SelfIni.GetSetting('Information', 'damage_count')) + HurtEvent.DamageAmount);
            SelfIni.Save();
 
            if(Data.GetConfigValue(ConfigName, "Cooldown", "enable") == "true")
            {
                var newer = TimeStamp() + Number(Data.GetConfigValue(ConfigName, "Cooldown", "attack_cooldown_in_second"));
                SetTabled('CoolDown', HurtEvent.Attacker.SteamID, newer);    
            }
        }
    }    
}
...
function On_PlayerHurt(HurtEvent) {
    if(Data.GetConfigValue(ConfigName, "Settings", "enable") == "true")
    {
        if(Data.GetConfigValue(ConfigName, "SafeZone", "enable") == "true")
        {
            if(SafeZone(HurtEvent.Victim) != false)
            {
                if(Data.GetConfigValue(ConfigName, "SafeZone", "pvp") == "false")
                {
                    var returno = true;
                } else {
                    var returno = false;    
            ...
            if(HurtEvent.Victim.SteamID != null)
            {
                ...
                if(inivictimget.GetSetting('Information', 'clan') != iniattackerget.GetSetting('Information', 'clan'))
                {
                    ...
                    if(Data.GetConfigValue(ConfigName, "Cooldown", "enable") == "true")
                    {
                        var newer = TimeStamp() + Number(Data.GetConfigValue(ConfigName, "Cooldown", "attack_cooldown_in_second"));
                        SetTabled('CoolDown', HurtEvent.Attacker.SteamID, newer);
                        SetTabled('CoolDown', HurtEvent.Victim.SteamID, newer);            
                    }                 
                }
