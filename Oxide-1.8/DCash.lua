-- Não mexer aqui!
PLUGIN.Title = "Cash"
PLUGIN.Description = "Sistema de cash vip. Compra e venda de itens. Dinheiro cash"
...
-- /cash - Mostra o tanto de cash que o player tem
-- /lcash - Mostra os itens da loja de cash
-- /ccash - Compra um item na loja de cash Ex: /ccash "Explosive Charge" 1
-- /vcash - Vende um item na loja de cash Ex: /vcash "Explosive Charge" 1
-- /addic - Adiciona um item na loja de cash Ex: /addic "Explosive Charge" "1000" "50"
-- /setcash - Dá cash pra um player
-- /cash top - Mostra os mais ricos em cash do servidor
-- /cash pagar - Dá dinheiro para outro player
-- /fcash - arruma a loja de cash
--
--
...
function PLUGIN:LoadDefaultConfig()
	...
	self.Config.Misc.AutoSaveTime = self.Config.Misc.AutoSaveTime or 60
	...
	self.Config.Econ.StartCash = self.Config.Econ.StartCash or 0
	self.Config.Econ.Symbol = self.Config.Econ.Symbol or "₡"
	self.Config.Econ.Rabbit = self.Config.Econ.Rabbit or 0
	self.Config.Econ.Wolf = self.Config.Econ.Wolf or 0
	self.Config.Econ.Bear = self.Config.Econ.Bear or 0
	self.Config.Econ.Chicken = self.Config.Econ.Chicken or 0
	self.Config.Econ.Stag = self.Config.Econ.Stag or 0
	self.Config.Econ.Boar = self.Config.Econ.Boar or 0
	self.Config.Econ.HumanPercent = self.Config.Econ.HumanPercent or 0
...
function PLUGIN:LoadCommands()
	if (self.Config.Enabled.Econ) then
		...
		self:AddChatCommand( "lcash", self.cmdPrice )
		self:AddChatCommand( "cash", self.cmdMoney )
		self:AddChatCommand( "bcash", self.cmdMoney )
		self:AddChatCommand( "ccash", self.cmdBuy )
		self:AddChatCommand( "vcash", self.cmdSell )
		self:AddChatCommand( "fcash", self.FixMoney )
	end
	if self.Config.Enabled.Econ then
	...
	if self.Config.Enabled.Econ then
		self:AddChatCommand( "addic", self.cmdAddPrice )
		self:AddChatCommand( "setcash", self.cmdSetMoney )
		self:AddChatCommand( "addcash", self.cmdAddMoney )
		self:AddCommand( "essentials", "addcash", self.ccmdAddMoney )
	end
...
function PLUGIN:DefaultEconConfig()
	self.CData["Explosive Charge"] = {Buy = 100, Sell = 50}
	self.CData["Bolt Action Rifle"] = {Buy = 50, Sell = 10}
	self.CData["Explosives"] = {Buy = 30, Sell = 15}
	self:EconSave()
end
...
function PLUGIN:EconConfig()
	if self.Config.Enabled.Econ then
		self.ConfigData = util.GetDatafile( "ItensCash" )
		if (self.ConfigData:GetText() == "") then
			self.CData = {}
			self:DefaultEconConfig()
			print( self.Title..": Tabela de cash criado" )
		else
			self.CData = json.decode( self.ConfigData:GetText() )
			if (not self.CData) then
				error( self.Title..": json error in ItensCash.txt" )
				self.CData = {}
			else
				print(self.Title..": Tabela de cash carregado.")
			end
		end
	end
end
...
function PLUGIN:cmdAddPrice(netuser, cmd, args)
	...
	if netuser:CanAdmin() or datan.Essentials == true or (flags ~= nil and flags:HasFlag(netuser, "econ")) then
		if not args[2] then
			rust.Notice(netuser, "Digite /addic \"NomeDoItem\" \"PrecoCompra\" \"PrecoVenda\"")
			return true
		end
		...
		self:EconSave()
		self:SendChatToUser(netuser,  "[color green] Voce adicionou/atualizou o item: "..itemname.." na loja de [color red] CASH!")
	else
...
function PLUGIN:cmdPrice( netuser, cmd, args, focus )
	...
	if type(pagenum) ~= "number" then
		...
		self:SendChatToUser(netuser,  "[color green]--- PAGINA [  "..tostring(pagenum).."/"..tostring(countpage).." ] Proxima pagina digite: /lcash 2 ou /lcash 3, etc..---")
		for k,value in pairsByKeys(self.CData) do
			count = count + 1
...
function PLUGIN:GetUserDataFromID( userID, name )
	...
	if (type(userentry) ~= "table" or userentry.DataVersion ~= self.DataVersion) then
		...
		userentry.Money = userentry.Money or self.Config.Econ.StartCash
...
function PLUGIN:cmdMoney(netuser, cmd, args)
	...
	elseif tostring(args[1]) == "pagar" then
		if not args[3] then
			rust.Notice(netuser, "Digite /cash pagar \"Player\" \"Quantidade\"")
			return true
		else
			...
			if netuser == targetuser then
				rust.Notice(netuser, "Voce nao pode dar cash a voce mesmo.")
				return false
			end
			if tonumber(args[3]) < 0 then
				rust.Notice(netuser, "Voce nao pode ficar com cash negativo.")
				return false
			end
	...
	elseif (args[1] and args[1] ~= "send" and args[1] ~= "top") then
		...
		self:SendChatToUser(netuser,  targetuser.displayName.." "..tostring(datat.Money)..self.Config.Econ.Symbol)
		return true
	end
end
...
function PLUGIN:ShowBalance(user)
	...
	self:SendChatToUser(user, " [color yellow] "..data.Money..self.Config.Econ.Symbol)
end
...
function PLUGIN:AddMoney(user, amount)
	...
	if type(data) == "table" then
		if amount == 0 then
			...
			self:SendChatToUser(user,"[color yellow]"..tostring(data.Money).." ( + "..tostring(amount)..self.Config.Econ.Symbol.." )")
			self:EconSave()
			return true
		end
	end
...
function PLUGIN:FixMoney(netuser)
	for _,v in pairs(self.PData.Users) do
		if v.Money < 0 then
			v.Money = self.Config.Econ.StartCash
		end
	end
	rust.Notice(netuser, "Cash arrumado!")
end
...
function PLUGIN:TakeMoney( user, amount)
	...
	if data.Money < amount then 
		rust.Notice(user, "Sem cash suficiente para fazer isso!")
		return false
	else
		amount = tonumber(amount)
		data.Money = data.Money - amount
		data.Money = math.floor(data.Money)
		self:SendChatToUser(user, "[color cyan]CASH: "..curmoney.." ( - "..amount..self.Config.Econ.Symbol.." )")
		return true
	end
end
...
function PLUGIN:cmdSetMoney( netuser, cmd, args )
	...
	if netuser:CanAdmin() or datan.Essentials == true or (flags ~= nil and flags:HasFlag(netuser, "econ")) then
		if not args[2] then
			rust.Notice(netuser, "Digite /setcash \"Player\" \"Quantidade\"")
		else
			local b, targetuser = rust.FindNetUsersByName( args[1] )
			if (not b) then
				if (targetuser == 0) then
...
function PLUGIN:cmdAddMoney(netuser, cmd, args)
	...
	if netuser:CanAdmin() or datan.Essentials == true or (flags ~= nil and flags:HasFlag(netuser, "econ")) then
		if (not args[2]) then
			rust.Notice(netuser, "Digite /addcash \"Player\" para dar cash.")
		end
		if args[2] then
			local b, targetuser = rust.FindNetUsersByName( args[1] )
			if (not b) then
...
function PLUGIN:ccmdAddMoney(arg)
	...
	if (arg:GetString(0) == nil) then
		arg:ReplyWith("Digite /addcash \"Player\" para dar CASH.")
	end
