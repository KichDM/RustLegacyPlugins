...
-- Load all the chat and console commands
-- ****************************
...
-- ****************************
-- PLUGIN:PostInit()
-- Do stuff after everything is loaded
-- ****************************
...
function PLUGIN:cmdBan(netuser, cmd, args)
    ...
    if(netuser and not self:CheckFlags(netuser)) then
...
function PLUGIN:cmdCheckBan(netuser, cmd, args)
    if((self:CheckFlags(netuser)) or (self.Config.Settings.CheckForEveryone == "true")) then
        if (type(cmd) == "table") then
            args = cmd
        end
        if(not args[1]) then
